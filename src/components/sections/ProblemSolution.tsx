  ...
  return (
    <section className="bg-ink-50 py-14 sm:py-16 lg:py-24">
      <Container className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center lg:gap-12">
        <SectionHeading
          eyebrow="რატომ TNT POWER"
          title="გაქვთ ბინა და არ იცით, საიდან დაიწყოთ რემონტი?"
          description="ხუთი ხელოსნის ნაცვლად — ერთი პასუხისმგებელი კომპანია. აი, რა იცვლება, როცა პროექტს TNT POWER-ს ანდობთ."
        />
                ...
                {comparisonRows.map((row) => (
                  <tr key={row.label}>
                    <td className="p-4 sm:p-5">
                      <span className="block font-medium leading-snug text-ink-950">{row.label}</span>
                      {row.hint && (
                        <span className="mt-0.5 block text-sm text-ink-600">{row.hint}</span>
                      )}
                    </td>
                    <td className="bg-brand-50 text-center">
                      <BrandCheck className="mx-auto" />
                      <span className="sr-only">დიახ</span>
                    </td>
                    <td className="text-center">
                      <X className="mx-auto size-5 text-ink-300" aria-hidden="true" />
                      <span className="sr-only">არა</span>
                    </td>
                  </tr>
                ))}
              </tbody>
