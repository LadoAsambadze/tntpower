...
export function PricingTable({ withHeading = true, limit }: PricingTableProps) {
      ...
      <Container>
        {withHeading && (
          <SectionHeading
            ...
            description="საორიენტაციო ფასები, რომ თავიდანვე იცოდეთ, რას უნდა ელოდოთ. ზუსტი თანხა — ობიექტის დათვალიერების შემდეგ."
          />
        )}
          ...
          <div className={withHeading ? "mt-10" : undefined}>
            ...
            >
              {rows.map((row) => (
                ...
                >
                  <p className="font-semibold leading-snug text-ink-950">{row.title}</p>
                  <p className="font-display mt-4 text-2xl font-semibold text-ink-950">{row.price}</p>
                  <p className="mt-2 flex-1 text-sm leading-relaxed text-ink-600">{row.note}</p>
            ...
            <ul className="divide-y divide-ink-200">
              {rows.map((row) => (
                ...
                >
                  ...
                  <span className="font-display text-lg font-semibold text-ink-950">{row.price}</span>
                  <span className="text-sm text-ink-600">{row.note}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-6 flex flex-col gap-4 rounded-xl border-l-4 border-brand-500 bg-ink-50 p-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          {limit ? (
            <Button href="/pricing" variant="dark" size="sm" className="shrink-0">
              ყველა ფასის ნახვა
            </Button>
          ) : (
            <Button href="/contact" size="sm" className="shrink-0">
              ხარჯთაღრიცხვის მოთხოვნა
            </Button>
          )}
        </div>
      </Container>
