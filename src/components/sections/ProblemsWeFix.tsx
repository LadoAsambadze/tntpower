...
async function ProblemCard({ problem: p }: { problem: Problem }) {
  const { tr, href } = await getI18n();
  const service = getService(p.service);
  return (
    <Link
      href={href(`/services/${p.service}`)}
    ...
    >
      <span className="flex items-start gap-3">
        ...
        <span className="min-w-0">
          <span className="block font-semibold leading-snug text-ink-950">{tr(p.problem)}</span>
          <span className="mt-0.5 block text-sm text-ink-600">{tr(p.symptom)}</span>
        </span>
      </span>
      ...
      <span className="mt-4 flex items-center gap-1 text-sm font-semibold text-ink-950 underline decoration-brand-500 decoration-2 underline-offset-4">
        {service ? tr(service.shortTitle) : tr("სერვისი")}
 ...
 */
export async function ProblemsWeFix() {
  const { tr, href } = await getI18n();
  return (
    <section className="bg-paper py-14 sm:py-16 lg:py-24">
      <Container className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center lg:gap-12">
        <div>
          <SectionHeading
            eyebrow={tr("იცნობთ ამ სიტუაციას?")}
            title={tr("პრობლემა, რომელიც დღეს გაწუხებთ — ჩვენი საქმეა")}
            description={tr("არ არის აუცილებელი, იცოდეთ, რომელი სპეციალისტი გჭირდებათ. აღწერეთ სიტუაცია — შესაბამის ჯგუფს ჩვენ ავარჩევთ.")}
          />
          <div className="mt-6 flex flex-col gap-3 sm:flex-row lg:flex-col xl:flex-row">
            <Button href={href("/contact")}>{tr("აღწერეთ თქვენი სიტუაცია")}</Button>
            <Button href={site.whatsappHref} variant="outline" target="_blank">
              {tr("გამოგზავნეთ ფოტო WhatsApp-ით")}
            </Button>
          </div>
        </div>

        <Carousel
          label={tr("ხშირი პრობლემები, რომლებსაც ვხსნით")}
