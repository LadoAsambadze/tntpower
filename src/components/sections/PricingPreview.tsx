      ...
      <Container className="relative grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-start lg:gap-14">
        <div>
          <SectionHeading
            ...
            description={tr("საორიენტაციო დიაპაზონები, რომ თავიდანვე იცოდეთ, რას უნდა ელოდოთ. ზუსტი თანხა — ობიექტის დათვალიერების შემდეგ.")}
          />
          ...
          <ul className="mt-8 divide-y divide-white/10 border-y border-white/10">
            {quickPrices.map((s) => (
              <li key={s.slug}>
                ...
                >
                  <span className="font-medium">{tr(s.shortTitle)}</span>
          ...
          <p className="mt-5 text-sm leading-relaxed text-ink-400">{tr(priceDisclaimer)}</p>
          ...
          >
            {tr("ყველა ფასი და კითხვა-პასუხი")}
